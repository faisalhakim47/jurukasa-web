...
describe('Purchase Creation View with Supplier Selector Dialog', function () {
  ...
  test('it shall open supplier selector dialog and select a supplier', async function ({ page }) {
    await Promise.all([
      ...
      setupDatabase(tursoLibSQLiteServer(), async function setupData(sql) {
        await sql`INSERT INTO suppliers (name, phone_number) VALUES ('Test Supplier', '+62812345678')`;
        await sql`INSERT INTO inventories (name, unit_price, unit_of_measurement, stock, account_code) VALUES ('Test Product', 10000, 'pcs', 100, 11310)`;
      }),
    ]);
  ...
  test('it shall allow changing supplier via edit button', async function ({ page }) {
    await Promise.all([
      loadEmptyFixture(page),
      setupDatabase(tursoLibSQLiteServer(), async function setupData(sql) {
        await sql`INSERT INTO suppliers (name, phone_number) VALUES ('Supplier A', '+62811111111')`;
        await sql`INSERT INTO suppliers (name, phone_number) VALUES ('Supplier B', '+62822222222')`;
      }),
    ]);
  ...
  test('it shall allow creating new supplier and automatically select it', async function ({ page }) {
    await Promise.all([
      loadEmptyFixture(page),
      setupDatabase(tursoLibSQLiteServer(), async function setupData(sql) {
        // No test data needed, just schema
      }),
    ]);
