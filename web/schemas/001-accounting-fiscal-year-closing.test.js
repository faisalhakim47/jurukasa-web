...
describe('Accounting Schema Tests - Fiscal Year Closing', function () {
  ...
  async function setupStandardClosingAccounts() {
    ...
    await db().execute("INSERT INTO chart_of_accounts_templates (name) VALUES ('Retail Business - Indonesia')");
  }
  ...
  describe('Multiple Revenue and Expense Accounts', function () {
    it('shall close fiscal year with multiple revenue accounts', async function () {
      ...
      const salesRevenue = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 41000`)).rows[0];
      const otherRevenue1 = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 81100`)).rows[0];
      const otherRevenue2 = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 81200`)).rows[0];
      ...
      const retainedEarnings = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 32000`)).rows[0];
    ...
    it('shall close fiscal year with multiple expense accounts', async function () {
      ...
      const cogs = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 51000`)).rows[0];
      const salaries = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 61300`)).rows[0];
      const rent = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 61100`)).rows[0];
      const utilities = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 61200`)).rows[0];
      const depreciation = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 61900`)).rows[0];
    ...
    it('shall close fiscal year with mixed revenues, expenses, and dividends', async function () {
      ...
      const salesRevenue = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 41000`)).rows[0];
      const otherRevenue = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 81100`)).rows[0];
      const cogs = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 51000`)).rows[0];
      const salaries = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 61300`)).rows[0];
      const rent = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 61100`)).rows[0];
      const dividends = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 33000`)).rows[0];
      ...
      const closingLines = (await db().execute(
        `SELECT COUNT(*) as count FROM journal_entry_lines WHERE journal_entry_ref = ?`,
  ...
  describe('Consecutive Fiscal Years', function () {
    it('shall close consecutive fiscal years correctly', async function () {
      ...
      await db().execute(
        `INSERT INTO fiscal_years (begin_time, end_time, name) VALUES (?, ?, ?)`,
        [fy2023Begin, fy2023End, 'FY2023']
      );

      const postTime2023 = new Date(2024, 0, 5, 0, 0, 0, 0).getTime();
      await db().execute(
        `UPDATE fiscal_years SET post_time = ? WHERE begin_time = ?`,
        [postTime2023, fy2023Begin]
      );

      // Verify FY2023 closing
      let retainedEarnings = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 32000`)).rows[0];
      ...
      await db().execute(
        `INSERT INTO fiscal_years (begin_time, end_time, name) VALUES (?, ?, ?)`,
        [fy2024Begin, fy2024End, 'FY2024']
      );

      const postTime2024 = new Date(2025, 0, 5, 0, 0, 0, 0).getTime();
      await db().execute(
        `UPDATE fiscal_years SET post_time = ? WHERE begin_time = ?`,
        [postTime2024, fy2024Begin]
      );

      // Verify cumulative retained earnings
      retainedEarnings = (await db().execute(`SELECT balance FROM accounts WHERE account_code = 32000`)).rows[0];
  ...
  describe('Validation Rules', function () {
    ...
    it('shall prevent modifying closed fiscal year attributes', async function () {
      ...
      await rejects(
        db().execute(
          `UPDATE fiscal_years SET post_time = NULL WHERE begin_time = ?`,
          [beginTime]
        ),
        /Cannot unpost or change post_time of a posted fiscal year/
      );
    ...
    it('shall enforce non-overlapping fiscal year constraint', async function () {
      ...
      await rejects(
        db().execute(
          `INSERT INTO fiscal_years (begin_time, end_time, name) VALUES (?, ?, ?)`,
          [beginTime1, endTime1, 'FY2024 Duplicate']
        ),
      ...
      await rejects(
        db().execute(
          `INSERT INTO fiscal_years (begin_time, end_time, name) VALUES (?, ?, ?)`,
          [beginTime2, endTime2, 'Overlapping FY']
        ),
        /Fiscal year periods cannot overlap/
      );
      ...
      await rejects(
        db().execute(
          `INSERT INTO fiscal_years (begin_time, end_time, name) VALUES (?, ?, ?)`,
          [beginTime3, endTime3, 'Overlapping FY2']
        ),
        /Fiscal year periods cannot overlap/
      );
    });
  ...
  describe('Closing Entry Details', function () {
    it('shall create closing entry with correct system metadata', async function () {
      ...
      const closingEntry = (await db().execute(
