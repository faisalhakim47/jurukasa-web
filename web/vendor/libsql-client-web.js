...
async function executeHranaBatch(mode, version$1, batch, hranaStmts, disableForeignKeys = false) {
	...
	const stmtPromises = hranaStmts.map((hranaStmt) => {
		const stmtStep = batch.step().condition(BatchCond$2.ok(lastStep));
		if (version$1 >= 3) stmtStep.condition(BatchCond$2.not(BatchCond$2.isAutocommit(batch)));
		const stmtPromise = stmtStep.query(hranaStmt);
		lastStep = stmtStep;
		return stmtPromise;
	});
	const commitStep = batch.step().condition(BatchCond$2.ok(lastStep));
	if (version$1 >= 3) commitStep.condition(BatchCond$2.not(BatchCond$2.isAutocommit(batch)));
...
//#endregion
export { createClient };
//# sourceMappingURL=libsql-client-web.js.map